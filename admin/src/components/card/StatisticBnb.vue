...
<template>
    <div class="flex items-center p-4 rounded-lg shadow-xs bg-gray-800">
        <div class="p-2 mr-4 rounded-full text-orange-100 bg-[#F0B90B]">
            <svg class="h-7 w-7" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 2496 2496" style="enable-background:new 0 0 2496 2496;" xml:space="preserve">
                <g>
        ...
        <div>
            <p class="flex items-center mb-2 text-sm font-medium whitespace-nowrap text-gray-400">
                {{ t('total-invested') }} (BNB) <span class="text-lg ml-1 font-semibold text-gray-200">{{ totalInvested.toFixed(2) }}</span>
            </p>

            <p class="flex items-center mb-2 text-sm font-medium whitespace-nowrap text-gray-400">
                {{ t('total-withdrawn') }} (BNB) <span class="text-lg ml-1 font-semibold text-gray-200">{{ totalWithdrawn.toFixed(2) }}</span>
            </p>
        </div>
    </div>
</template>